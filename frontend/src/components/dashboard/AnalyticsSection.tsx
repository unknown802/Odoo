...
export function AnalyticsSection({ assets, departments }: AnalyticsSectionProps) {
  ...
  const utilizationData = ["Jan", "Feb", "Mar", "Apr", "May", "Jun"].map((name, index) => {
    const trendOffset = 5 - index;
    const allocated = Math.max(0, Math.min(currentTotal, currentAllocated - Math.ceil(trendOffset / 2)));
    return { name, allocated, available: Math.max(0, currentTotal - allocated) };
  });
