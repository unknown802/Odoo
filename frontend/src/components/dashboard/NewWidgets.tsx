import { CheckCircle2, AlertTriangle, ShieldCheck, Activity, ChevronRight, ShieldAlert, FileText, BellRing } from "lucide-react";
import { Card } from "../ui/Card";
import { Badge } from "../ui/Badge";
import type { MaintenanceRequest, TransferRequest, NotificationItem } from "../../types";
...
export function PendingApprovalsWidget({ transfers }: { transfers: TransferRequest[] }) {
            ...
            {pending.slice(0, 4).map(t => (
              <div key={t.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer border border-transparent hover:border-slate-100">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-700 truncate">Transfer Request</p>
                  <p className="text-xs text-slate-500 truncate mt-0.5">from {t.from_holder_id || 'unassigned'} to {t.to_holder_id}</p>
                </div>
